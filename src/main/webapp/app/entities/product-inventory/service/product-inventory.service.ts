...
export class ProductInventoryService {
  ...
  addProductInventoryToCollectionIfMissing<Type extends Pick<IProductInventory, 'id'>>(
    ...
    if (productInventories.length > 0) {
      const productInventoryCollectionIdentifiers = productInventoryCollection.map(productInventoryItem =>
        this.getProductInventoryIdentifier(productInventoryItem),
      );
